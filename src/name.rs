...
impl<'a> NameTable<'a> {
    pub fn parse_name_record(&self, input: NameRecord) -> UTF16BE<'a> {
        match self {
...
impl<'a> NameTableVersion0<'a> {
    pub fn parse_name_record(&self, input: NameRecord) -> UTF16BE<'a> {
        ...
        let bytes = &self.storage[offset..offset + length];
        UTF16BE::new(bytes)
    }
}
...
impl<'a> NameTableVersion1<'a> {
    ...
    pub fn parse_lang_tag_record(&self, input: LangTagRecord) -> UTF16BE<'a> {
...
impl NameRecord {
    /// Copyright notice.
    pub fn is_copyright_notice(&self) -> bool {
    ...
    pub fn is_font_family_name(&self) -> bool {
    ...
    pub fn is_font_subfamily_name(&self) -> bool {
        self.name_id.into_u16() == 2
    }

    /// Unique font identifier.
    pub fn is_unique_font_identifier(&self) -> bool {
    ...
    pub fn is_full_font_name(&self) -> bool {
    ...
    pub fn is_version_string(&self) -> bool {
    ...
    pub fn is_postscript_name(&self) -> bool {
    ...
    /// Manufacturer Name.
    pub fn is_manufacturer_name(&self) -> bool {
    ...
    pub fn is_typographic_family_name(&self) -> bool {
    ...
    pub fn is_typographic_subfamily_name(&self) -> bool {
    ...
    pub fn is_compatible_full(&self) -> bool {
    ...
    pub fn is_sample_text(&self) -> bool {
    ...
    pub fn is_postscript_cid_findfont_name(&self) -> bool {
    ...
    pub fn is_wws_family_name(&self) -> bool {
    ...
    ///
    /// See additional remarks regarding IDs 21 and 22 below.
    pub fn is_wws_subfamily_name(&self) -> bool {
    ...
    pub fn is_light_background_palette(&self) -> bool {
    ...
    pub fn is_dark_background_palette(&self) -> bool {
    ...
    pub fn is_variations_postscript_name_prefix(&self) -> bool {
...
impl<'a> NameParser<'a> {
    ...
    pub(crate) fn parse(&mut self) -> Option<NameTable<'a>> {
        let version = self.stream.parse_u16()?;
        match version {
            0 => Some(NameTable::Version0(self.parse_version_0(version)?)),
            1 => Some(NameTable::Version1(self.parse_version_1(version)?)),
            _ => None,
        }
    }

    pub(crate) fn parse_version_0(&mut self, version: u16) -> Option<NameTableVersion0<'a>> {
        let count = self.stream.parse_u16()?;
        let storage_offset = self.stream.parse_u16()?;
        let name_record = self.stream.parse_slice(count as usize)?;
    ...
    pub(crate) fn parse_version_1(&mut self, version: u16) -> Option<NameTableVersion1<'a>> {
        let count = self.stream.parse_u16()?;
        let storage_offset = self.stream.parse_u16()?;
        let name_record = self.stream.parse_slice(count as usize)?;
        let lang_tag_count = self.stream.parse_u16()?;
        let lang_tag_record = self.stream.parse_slice(lang_tag_count as usize)?;
        let storage = self.stream.parse_slice_rest();
...
impl<'a> Parser<'a> {
    pub fn parse_name(&self, input: TableRecord) -> Option<NameTable<'a>> {
